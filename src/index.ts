...
const STATIC_RESPONSE = new Response("Cron worker is active and running perfectly! 😊", { status: 200 });
...
export default {
	...
	async scheduled(_event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
		...
		if (!env.RENDER_HEALTHCHECK_URL.startsWith('https:')) {
			console.error("RENDER_HEALTHCHECK_URL must use the https: protocol.");
			return;
		}
